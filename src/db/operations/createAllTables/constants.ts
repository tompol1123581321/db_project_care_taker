export const CREATE_TABLE_ARGS: {
...
}[] = [
  {
    name: "buildings",
    definition:
      "id INT AUTO_INCREMENT PRIMARY KEY, address VARCHAR(255), date DATE, amountOfFlats INT, representativeId INT, FOREIGN KEY (representativeId) REFERENCES owners(id)",
    hasForeignKey: true,
  },
  {
    name: "flatTypes",
    definition:
      "id INT AUTO_INCREMENT PRIMARY KEY, size ENUM( '1+1', '2+1', '3+1', '4+1' ) NOT NULL, rented ENUM( 'true', 'false' )",
  },
  {
    name: "flats",
    definition:
      "id INT AUTO_INCREMENT PRIMARY KEY, ownerId INT, flatTypeId INT, buildingId INT, FOREIGN KEY (ownerId) REFERENCES owners(id), FOREIGN KEY (flatTypeId) REFERENCES flatTypes(id), FOREIGN KEY (buildingId) REFERENCES buildings(id)",
    hasForeignKey: true,
  },
  {
    name: "owners",
    definition:
      "id INT AUTO_INCREMENT PRIMARY KEY, address VARCHAR(255), name VARCHAR(255), birthDate DATE, contact VARCHAR(255)",
  },
];
...
> = {
  buildings: {
    sql: "INSERT INTO buildings (address, date, amountOfFlats, representativeId) VALUES ?",
    data: [
      ["Ap #485-9519 Sed Rd.", "Jan 21, 2002", 10, 1],
      ["P.O. Box 272, 3580 Cursus Rd.", "Oct 11, 1999", 10, 15],
      ["507-4224 Ultrices Ave", "Oct 3, 2003", 20, 40],
  ...
  owners: {
    sql: "INSERT INTO owners (address, name, birthDate, contact) VALUES ?",
    data: [
      [
        "Ap #485-9519 Sed Rd.",
        "Freya Huber",
